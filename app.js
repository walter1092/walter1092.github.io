const TripExpenseTracker = () => {
  ...
  // Add this at the very beginning of your component (after db initialization)
  React.useEffect(() => {
  ...
  // State for people
  const [people, setPeople] = React.useState(['Person 1', 'Person 2']);
    ...
    try {
      ...
      if (lastAuth && storedPassword) {
        ...
        if (now - authTime < 24 * 60 * 60 * 1000) {
          ...
          // Add this line to load data on auto-login
          loadDataFromFirebase(storedPassword);
        }
      }
    } catch (error) {
  ...
  // Add this effect somewhere with your other effects
  React.useEffect(() => {
    if (isAuthenticated && storedPassword) {
      saveDataToFirebase();
    }
  }, [people, expenses, customSplits]); // This will trigger when any of these change
  ...
  const handleLogin = () => {
    try {
      ...
      localStorage.setItem('trip-tracker-password', password);
      localStorage.setItem('trip-tracker-auth-time', new Date().getTime().toString());
      
      // Load data from Firebase based on password
      loadDataFromFirebase(password);
    } catch (error) {
  ...
  const loadDataFromFirebase = async (tripPassword) => {
    try {
      console.log('Attempting to load data from Firebase with password:', tripPassword);
      
      const docRef = db.collection('trips').doc(tripPassword);
      const doc = await docRef.get();
      
      if (doc.exists) {
        console.log('Found data in Firebase:', doc.data());
        const data = doc.data();
        const loadedPeople = data.people || ['Person 1', 'Person 2'];
        const loadedExpenses = data.expenses || [];
        const loadedSplits = data.customSplits || {};
        
        // Check if expenses need updating (paidBy referencing people that don't exist)
        const updatedExpenses = loadedExpenses.map(expense => {
          const newExpense = {...expense};
          
          // If paidBy isn't in the people list, assign to first person
          if (!loadedPeople.includes(expense.paidBy)) {
            newExpense.paidBy = loadedPeople[0];
          }
          
          // Update splits to only include current people
          const updatedSplits = {};
          let splitTotal = 0;
          
          // Keep splits for existing people
          Object.entries(expense.splits).forEach(([person, percentage]) => {
            if (loadedPeople.includes(person)) {
              updatedSplits[person] = percentage;
              splitTotal += percentage;
            }
          });
          
          // If splits don't add up to 100%, distribute remainder evenly
          if (Math.abs(splitTotal - 100) > 0.01) {
            const remainingPercent = 100 - splitTotal;
            const perPersonAdjustment = remainingPercent / loadedPeople.length;
            
            loadedPeople.forEach(person => {
              updatedSplits[person] = (updatedSplits[person] || 0) + perPersonAdjustment;
            });
          }
          
          newExpense.splits = updatedSplits;
          return newExpense;
        });
        ...
        console.log('Data loaded and migrated successfully from Firebase');
        
        // Save the migrated data back if changes were made
        if (JSON.stringify(loadedExpenses) !== JSON.stringify(updatedExpenses)) {
          setTimeout(() => saveDataToFirebase(), 500);
        }
      } else {
        console.log('No data found in Firebase for this password');
      }
    } catch (error) {
  ...
  const saveDataToFirebase = () => {
    if (!storedPassword) {
      console.log('No password set, not saving to Firebase');
      return;
    }
    
    console.log('Attempting to save to Firebase with password:', storedPassword);
    console.log('Data to save:', { people, expenses, customSplits });
    
    db.collection('trips').doc(storedPassword).set({
      people,
      expenses,
      customSplits,
  ...
  // Fix expense data to match current people
  const fixExpenseData = () => {
    // Only proceed if there are expenses and people
    if (expenses.length === 0 || people.length === 0) return;
    
    // Update expenses to use valid people
    const updatedExpenses = expenses.map(expense => {
      const newExpense = {...expense};
      
      // If paidBy isn't in the people list, assign to first person
      if (!people.includes(expense.paidBy)) {
        newExpense.paidBy = people[0];
      }
      
      // Make sure splits only include current people
      const updatedSplits = {};
      people.forEach(person => {
        // Keep existing split or default to equal split
        updatedSplits[person] = expense.splits[person] || (100 / people.length);
      });
      
      // Normalize splits to 100%
      const totalSplit = Object.values(updatedSplits).reduce((sum, val) => sum + val, 0);
      if (Math.abs(totalSplit - 100) > 0.01) {
        const factor = 100 / totalSplit;
        Object.keys(updatedSplits).forEach(person => {
          updatedSplits[person] *= factor;
        });
      }
    ...
    // Update expenses state and save to Firebase
    setExpenses(updatedExpenses);
    saveDataToFirebase();
  };
  ...
  const calculateBalances = () => {
    ...
    expenses.forEach(expense => {
      ...
      // Handle case where paidBy is not in current people list
      if (people.includes(paidBy)) {
        // Add the full amount to the person who paid
        balances[paidBy] += amount;
      } else {
        console.log(`Warning: Expense paid by "${paidBy}" who is not in current people list`);
        // Skip crediting this expense since we don't know who paid
      }
      ...
      {!isAuthenticated ? (
        ...
        <>
          ...
          <div className="mb-8 p-4 bg-gray-50 rounded-lg shadow">
            ...
            {people.some(p => p.startsWith('Person ')) && (
              <div className="mb-4 p-3 bg-yellow-100 text-yellow-800 rounded">
                ⚠️ You're using default person names. Consider updating them to real names.
              </div>
            )}
            ...
            {/* Reset button here */}
            {expenses.length === 1 && expenses[0].description === 'Hotel' && (
              <button
                onClick={() => {
                  if (window.confirm('Reset all expense data? This will keep your people but remove all expenses.')) {
                    setExpenses([]);
                    saveDataToFirebase();
                  }
                }}
                className="w-full mt-2 px-4 py-2 text-red-600 border border-red-600 rounded hover:bg-red-50"
              >
                Reset Data
              </button>
            )}
...
// Render the component - remove the export default
